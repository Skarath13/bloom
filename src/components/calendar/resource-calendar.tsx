...
export function ResourceCalendar({
  ...
  // Pinch-to-zoom - store setZoomLevel in a ref so effect doesn't re-run on zoom changes
  const setZoomLevelRef = useRef(config.setZoomLevel);
  setZoomLevelRef.current = config.setZoomLevel;

  // iOS Safari pinch-to-zoom using gesture events
  // Uses CSS transform during gesture for smooth visual feedback (GPU accelerated)
  // Only commits actual zoom level on gesture end to avoid React re-render stutter
  useEffect(() => {
    if (!config.isMobile) return;
    const element = gridRef.current;
    if (!element) return;

    // Check if Safari gesture events are supported
    const supportsGestureEvents = 'GestureEvent' in window;
    if (!supportsGestureEvents) return;

    // Find the content div inside the scroll container
    const contentDiv = element.querySelector(':scope > div') as HTMLElement | null;
    ...
    const handleGestureStart = (e: Event) => {
      ...
      // Read current zoom from localStorage
      const saved = localStorage.getItem('calendar-zoom');
      startZoom = saved ? parseFloat(saved) : 1.0;
    ...
    const handleGestureChange = (e: Event) => {
      ...
      // Calculate target zoom (clamped)
      const targetZoom = Math.min(1.5, Math.max(0.6, startZoom * ge.scale));
      // Calculate visual scale relative to current rendered zoom
      currentScale = targetZoom / startZoom;

      // Apply CSS transform for smooth visual scaling (GPU accelerated, no reflow)
      contentDiv.style.transform = `scaleY(${currentScale})`;
      contentDiv.style.transformOrigin = 'top center';

      // Adjust scroll to maintain position
      element.scrollTop = startScrollTop * currentScale;
    };
    ...
    const handleGestureEnd = (e: Event) => {
      ...
      const finalZoom = Math.min(1.5, Math.max(0.6, startZoom * currentScale));
      ...
      // Commit the actual zoom level (triggers React re-render)
      setZoomLevelRef.current(finalZoom);
    };
